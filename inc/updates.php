 ...
 * call our filter.
 */
const PHILOSOPHY_BLOCKS_UPDATE_HOST     = 'updates.colorlib.com';
const PHILOSOPHY_BLOCKS_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/philosophy-blocks.json';
const PHILOSOPHY_BLOCKS_UPDATE_CACHE    = 'philosophy_blocks_update_response';
 ...
 * @return bool
 */
function philosophy_blocks_updates_enabled() {
	/**
	 * Filters whether Philosophy Blocks checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'philosophy_blocks_check_for_updates', true );
}
 ...
 * @return array
 */
function philosophy_blocks_update_payload() {
	$payload = array(
		'theme'      => 'philosophy-blocks',
		'version'    => PHILOSOPHY_BLOCKS_VERSION,
	 ...
	 */
	return apply_filters( 'philosophy_blocks_update_payload', $payload );
}
 ...
 * @return array|null
 */
function philosophy_blocks_fetch_update() {
	if ( ! philosophy_blocks_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( PHILOSOPHY_BLOCKS_UPDATE_CACHE );
	...
	$response = wp_remote_get(
		add_query_arg( philosophy_blocks_update_payload(), PHILOSOPHY_BLOCKS_UPDATE_ENDPOINT ),
		array(
			'timeout'    => 8,
			'user-agent' => philosophy_blocks_update_user_agent(),
		)
	);

	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( PHILOSOPHY_BLOCKS_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( PHILOSOPHY_BLOCKS_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function philosophy_blocks_check_update( $update, $theme_data, $theme_stylesheet ) {
	if ( $update || 'philosophy-blocks' !== $theme_stylesheet ) {
		return $update;
	}

	$release = philosophy_blocks_fetch_update();

	if ( ! $release || version_compare( $release['version'], PHILOSOPHY_BLOCKS_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => PHILOSOPHY_BLOCKS_UPDATE_ENDPOINT,
...
add_filter( 'update_themes_' . PHILOSOPHY_BLOCKS_UPDATE_HOST, 'philosophy_blocks_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function philosophy_blocks_flush_update_cache() {
	delete_site_transient( PHILOSOPHY_BLOCKS_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'philosophy_blocks_flush_update_cache' );
add_action( 'after_switch_theme', 'philosophy_blocks_flush_update_cache' );
 ...
 * their server.
 */
function philosophy_blocks_updates_notice() {
	...
	$payload = philosophy_blocks_update_payload();
	?>
	<p class="description philosophy-updates-note">
		<?php
		printf(
			/* translators: 1: the update host, 2: the list of values sent, 3: filter name. */
			esc_html__( 'Philosophy Blocks checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'philosophy-blocks' ),
			'<code>' . esc_html( PHILOSOPHY_BLOCKS_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>philosophy_blocks_check_for_updates</code>'
		);
		?>
	</p>
...
add_action( 'admin_footer', 'philosophy_blocks_updates_notice' );
 ...
 * @return string
 */
function philosophy_blocks_update_user_agent() {
	return 'Philosophy Blocks/' . PHILOSOPHY_BLOCKS_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function philosophy_blocks_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = philosophy_blocks_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'philosophy_blocks_update_request_args', 10, 2 );

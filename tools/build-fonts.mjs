/**
 * Downloads the theme's webfonts and writes the @font-face stylesheet for the
 * subsets theme.json does not carry.
 *
 * theme.json registers the latin faces, because those are the ones the Site
 * Editor offers and applies. This script downloads those files too, and then
 * writes assets/css/fonts.css with only the *extra* subsets — latin-ext, and
 * the Montserrat fallback — so nothing is declared twice.
 *
 * Three families, and the reason for each:
 *
 * - **Metropolis** is the sans the Philosophy design was drawn with, in the
 *   HTML template and in the classic theme up to 1.1.x. It is not on Google
 *   Fonts, so it comes from Fontsource. Latin only.
 * - **Libre Baskerville** supplies the headings. Its bold has to come from
 *   Fontsource as well: the Google Fonts CSS API serves *the same file* for
 *   weight 400 and weight 700, so a theme built from it declares a bold it does
 *   not have and every heading is synthesised by the browser.
 ...
 *   Ș ș Ț ț. A page with no such character never downloads it.
 *
...
/** Faces theme.json already declares, and so must not be repeated here. */
const IN_THEME_JSON = ( family, subset ) => 'latin' === subset;
...
for ( const { pkg, version, family, faces } of FONTSOURCE ) {
	for ( const [ subset, weight, style ] of faces ) {
		...
		if ( ! IN_THEME_JSON( family, subset ) ) {
...
const header = `/**
 * Webfont subsets theme.json does not register for Philosophy Blocks.
 *
